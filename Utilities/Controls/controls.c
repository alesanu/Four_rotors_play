...
radio_command_list_t* radio_command_list_start = 0;
//radio_command_list_t* radio_command_list_end = 0;

uint8_t command_parse(command_t* command) {
	radio_command_list_t* orig = 0;
  orig = radio_command_list_start;
  if (orig == 0) {
    STM_EVAL_LEDOn(LED4);
    return 0;
  }/*
  ...
  command_radio_control(command);
  radio_command_list_start = orig->next;
  free(orig->radio);
  free(orig);
...
void command_radio_control(command_t* command) {
  memcpy(&command->throttle, &radio_command_list_start->radio[RADIO_THROTTLE], sizeof(float));
  memcpy(&command->roll, &radio_command_list_start->radio[RADIO_ROLL], sizeof(float));
  memcpy(&command->pitch, &radio_command_list_start->radio[RADIO_PITCH], sizeof(float));
  memcpy(&command->yaw, &radio_command_list_start->radio[RADIO_YAW], sizeof(float));
}
